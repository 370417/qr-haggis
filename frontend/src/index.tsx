    ...
    class CardGrid extends React.Component<CardGridProps> {
        render() {
            ...
            for (let key = 0; key < 36; key++) {
                normalCards.push(<Card key={key} frontendState={module.CardFrontendState.Unknown} cardId={key} selected={false} stage={this.props.stage} />);
            }
            for (let key = 36; key < 39; key++) {
                myWilcards.push(<Card key={key} frontendState={module.CardFrontendState.Unknown} cardId={key} selected={false} stage={this.props.stage} />);
            }
            for (let key = 39; key < 42; key++) {
                opponentWildcards.push(<Card key={key} frontendState={module.CardFrontendState.Unknown} cardId={key} selected={false} stage={this.props.stage} />);
            }
